...
def main():
    ...
    parser.add_option("-n","--indexname",action="store",dest="indexname",help="")
    
    (options,args) = parser.parse_args()

    for key in ([options.server, options.coll, options.indexname]):
        if not (key):
            parser.print_help()
            sys.exit(0)
